...
LONG RegisterAsioDriver(CLSID clsid, const char *szdllname, const char *szregname, const char *szasiodesc, const char *szthreadmodel);

LONG UnregisterAsioDriver(CLSID clsid, const char *szdllname, const char *szregname);
...
static LONG createRegStringValue(HKEY mainkey, const char *szregpath, const char *szvalstr, const char *valname);
...
LONG RegisterAsioDriver(CLSID clsid, const char *szdllname, const char *szregname, const char *szasiodesc, const char *szthreadmodel) {
    ...
    DBG_CTX(register ::RegisterAsioDriver, "clsid=[{}-{}-{}-{}], dllname={}, regname={}, asiodesc={}, threadmodel={}",
            clsid.Data1, clsid.Data2, clsid.Data3, clsid.Data4, szdllname, szregname, szasiodesc, szthreadmodel);


    hMod = GetModuleHandle(szdllname);
    if (!hMod) {
        MessageBox(0, (LPCTSTR) "ERRSREG_MODULE_NOT_FOUND", (LPCTSTR) "ASIO sample Driver", MB_OK);
        return ERRSREG_MODULE_NOT_FOUND;
    }
    szModuleName[0] = 0;
    GetModuleFileName(hMod, szModuleName, MAX_PATH_LEN);
    if (!szModuleName[0]) {
        MessageBox(0, (LPCTSTR) "ERRSREG_MODPATH_NOT_FOUND", (LPCTSTR) "ASIO sample Driver", MB_OK);
    ...
    if (rc != S_OK) {
        MessageBox(0, (LPCTSTR) "ERRSREG_STRING_FROM_CLSID", (LPCTSTR) "ASIO sample Driver", MB_OK);
        return ERRSREG_STRING_FROM_CLSID;
    }
    rc = (LONG) WideCharToMultiByte(CP_ACP, 0, (LPWSTR) wszCLSID, -1, szclsid, CLSID_STRING_LEN, 0, 0);
    if (!rc) {
        MessageBox(0, (LPCTSTR) "ERRSREG_CHAR_TO_MULTIBYTE", (LPCTSTR) "ASIO sample Driver", MB_OK);
    ...
    if (newregentry) {
        ...
        sprintf(szregpath, "%s\\%s", SZREGSTR_CLSID, szclsid);
        createRegStringValue(HKEY_CLASSES_ROOT, szregpath, szasiodesc, 0);
        // HKEY_CLASSES_ROOT\CLSID\InProcServer32
        sprintf(szregpath, "%s\\%s", SZREGSTR_CLSID, szclsid);
        createRegPath(HKEY_CLASSES_ROOT, szregpath, SZREGSTR_INPROCSERVER32);
        // HKEY_CLASSES_ROOT\CLSID\InProcServer32 -> DLL path
        sprintf(szregpath, "%s\\%s\\%s", SZREGSTR_CLSID, szclsid, SZREGSTR_INPROCSERVER32);
        createRegStringValue(HKEY_CLASSES_ROOT, szregpath, szModuleName, 0);
        // HKEY_CLASSES_ROOT\CLSID\InProcServer32 -> ThreadingModel
        createRegStringValue(HKEY_CLASSES_ROOT, szregpath, szthreadmodel, SZREGSTR_THREADINGMODEL);
    }
    ...
    sprintf(szregpath, "%s\\%s\\%s", SZREGSTR_SOFTWARE, SZREGSTR_ASIO, szregname);
    createRegStringValue(HKEY_LOCAL_MACHINE, szregpath, szclsid, SZREGSTR_CLSID);
    createRegStringValue(HKEY_LOCAL_MACHINE, szregpath, szasiodesc, SZREGSTR_DESCRIPTION);
...
LONG UnregisterAsioDriver(CLSID clsid, const char *szdllname, const char *szregname) {
    ...
    DBG_CTX(register ::UnregisterAsioDriver, "clsid=[{}-{}-{}-{}], dllname={}, regname={}", clsid.Data1, clsid.Data2,
            clsid.Data3, clsid.Data4, szdllname, szregname);
